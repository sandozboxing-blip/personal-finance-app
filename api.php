...
function emptyState(): array { return ['version'=>2,'leads'=>[],'smm'=>[],'web'=>[],'tasks'=>[],'userData'=>[]]; }
...
if($action==='load'&&$_SERVER['REQUEST_METHOD']==='GET'){
  ...
  reply(['ok'=>true,'state'=>['version'=>2,'updatedAt'=>$state['updatedAt']??null,'leads'=>is_array($state['leads']??null)?$state['leads']:[],'smm'=>is_array($state['smm']??null)?$state['smm']:[],'web'=>is_array($state['web']??null)?$state['web']:[],'tasks'=>is_array($profile['tasks']??null)?$profile['tasks']:($user==='Admin'?$legacyTasks:[]),'taskCategories'=>is_array($profile['taskCategories']??null)?$profile['taskCategories']:[],'settings'=>is_array($profile['settings']??null)?$profile['settings']:[]]]);
}
if($action==='save'&&$_SERVER['REQUEST_METHOD']==='POST'){
  $incoming=body();foreach(['leads','smm','web','tasks','taskCategories'] as $key){if(!isset($incoming[$key])||!is_array($incoming[$key]))$incoming[$key]=[];}if(!isset($incoming['settings'])||!is_array($incoming['settings']))$incoming['settings']=[];
  $file=dataFile();$fh=fopen($file,'c+');if(!$fh)reply(['ok'=>false,'error'=>'Папка data няма права за запис'],500);flock($fh,LOCK_EX);rewind($fh);$existing=decodeState((string)stream_get_contents($fh));$userData=is_array($existing['userData']??null)?$existing['userData']:[];$user=(string)$_SESSION['user'];$userData[$user]=['tasks'=>$incoming['tasks'],'taskCategories'=>$incoming['taskCategories'],'settings'=>$incoming['settings'],'updatedAt'=>gmdate('c')];
  $state=['version'=>2,'updatedAt'=>gmdate('c'),'updatedBy'=>$user,'leads'=>$incoming['leads'],'smm'=>$incoming['smm'],'web'=>$incoming['web'],'tasks'=>[],'userData'=>$userData];$json=json_encode($state,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);if($json===false||strlen($json)>20*1024*1024){flock($fh,LOCK_UN);fclose($fh);reply(['ok'=>false,'error'=>'Невалидни или твърде големи данни'],400);}ftruncate($fh,0);rewind($fh);fwrite($fh,"<?php exit; ?>\n".$json);fflush($fh);flock($fh,LOCK_UN);fclose($fh);reply(['ok'=>true,'updatedAt'=>$state['updatedAt']]);
}
