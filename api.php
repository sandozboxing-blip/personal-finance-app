...
function emptyState(): array { return ['version'=>7,'leads'=>[],'leadFolders'=>[],'smm'=>[],'web'=>[],'workTasks'=>[],'tasks'=>[],'userData'=>[]]; }
...
if($action==='load'&&$_SERVER['REQUEST_METHOD']==='GET'){
  ...
  reply(['ok'=>true,'state'=>['version'=>7,'updatedAt'=>$state['updatedAt']??null,'leads'=>is_array($state['leads']??null)?$state['leads']:[],'leadFolders'=>is_array($state['leadFolders']??null)?$state['leadFolders']:[],'smm'=>is_array($state['smm']??null)?$state['smm']:[],'web'=>is_array($state['web']??null)?$state['web']:[],'workTasks'=>is_array($state['workTasks']??null)?$state['workTasks']:[],'tasks'=>is_array($profile['tasks']??null)?$profile['tasks']:($user==='Admin'?$legacyTasks:[]),'focusTasks'=>is_array($profile['focusTasks']??null)?$profile['focusTasks']:[],'focusInitialized'=>array_key_exists('focusTasks',$profile),'taskCategories'=>is_array($profile['taskCategories']??null)?$profile['taskCategories']:[],'settings'=>is_array($profile['settings']??null)?$profile['settings']:[]]]);
}
if($action==='save'&&$_SERVER['REQUEST_METHOD']==='POST'){
  $incoming=body();$scope=(string)($incoming['saveScope']??'all');if(!in_array($scope,['shared','profile','all'],true))$scope='all';
  foreach(['leads','leadFolders','smm','web','workTasks','tasks','focusTasks','taskCategories'] as $key){if(!isset($incoming[$key])||!is_array($incoming[$key]))$incoming[$key]=[];}if(!isset($incoming['settings'])||!is_array($incoming['settings']))$incoming['settings']=[];
  ...
  $state=['version'=>7,'updatedAt'=>gmdate('c'),'updatedBy'=>$user,'leads'=>$preserveShared?(is_array($existing['leads']??null)?$existing['leads']:[]):$incoming['leads'],'leadFolders'=>$preserveShared?(is_array($existing['leadFolders']??null)?$existing['leadFolders']:[]):$incoming['leadFolders'],'smm'=>$preserveShared?(is_array($existing['smm']??null)?$existing['smm']:[]):$incoming['smm'],'web'=>$preserveShared?(is_array($existing['web']??null)?$existing['web']:[]):$incoming['web'],'workTasks'=>$preserveShared?(is_array($existing['workTasks']??null)?$existing['workTasks']:[]):$incoming['workTasks'],'tasks'=>[],'userData'=>$userData];$json=json_encode($state,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
