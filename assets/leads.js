...
function renderLeads() {
  ...
  leadVisibleIds=fil.map(function(l){return l.id;});var campaignBtn=document.getElementById('leadCampaignsBtn');if(campaignBtn){var folderCampaigns=leads.filter(function(l){return l.folder===folder&&l.outreach&&(l.outreach.status==='sent'||l.outreach.status==='replied');}),replyCount=folderCampaigns.filter(function(l){return l.outreach.status==='replied';}).length;campaignBtn.textContent='Кампании '+folderCampaigns.length+(replyCount?' · '+replyCount+' отговора':'');campaignBtn.style.display=folderHome?'none':'';}
...
function openLeadOutreach(){var folder=getSelectedLeadFolder();if(!folder){toast('Първо отвори папка с leads','var(--yellow)');return;}outreachScopeIds=leadVisibleIds.slice();var ov=document.getElementById('leadOutreachOv');if(!ov)return;document.getElementById('leadOutreachTitle').textContent='Кампания · '+folder+' · '+outreachScopeIds.length+' филтрирани';ov.classList.add('open');document.body.classList.add('outreach-open');outreachSegment='no_presence';var rows=outreachRows(outreachSegment);outreachLeadId=rows.length?rows[0].id:null;renderLeadOutreach();}
