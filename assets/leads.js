...
function renderLeadFolderView(active){var box=document.getElementById('leadFolderView');if(!box)return;var names=getLeadFolderNames();if(active){var count=leads.filter(function(l){return l.folder===active;}).length;box.className='leadfolderview compact';box.innerHTML='<button class="folderback" onclick="showLeadFolders()">← Всички папки</button><div><span>ОТВОРЕНА ПАПКА</span><h2>'+esc(active)+'</h2></div><b>'+count+' leads</b>';return;}box.className='leadfolderview';var cards=names.map(function(name){var rows=leads.filter(function(l){return l.folder===name;}),sites=rows.filter(function(l){return l.website;}).length,contacts=rows.filter(function(l){return l.phone||l.email;}).length;return'<button class="leadfoldercard" data-folder="'+esc(name)+'" onclick="openLeadFolder(this.dataset.folder)"><span class="foldericon">▰</span><span class="foldercopy"><strong>'+esc(name)+'</strong><small>'+rows.length+' leads</small></span><span class="folderstats"><i>'+sites+' със сайт</i><i>'+contacts+' с контакт</i></span><span class="folderarrow">→</span></button>';}).join('');box.innerHTML='<div class="folderhomehead"><div><span>LEADS БИБЛИОТЕКА</span><h2>Папки с потенциални клиенти</h2><p>Избери папка, за да видиш бизнесите в нея.</p></div><button class="btn btnp" onclick="createLeadFolder()">＋ Създай нова папка</button></div><div class="leadfoldergrid">'+cards+'<button class="leadfoldercard create" onclick="createLeadFolder()"><span class="foldericon">＋</span><span class="foldercopy"><strong>Нова папка</strong><small>Организирай следващото търсене</small></span></button></div>';}function parseReviewCount(v){var x=String(v||'').replace(/[()\s,]/g,'').toUpperCase(),m=parseFloat(x)||0;return x.indexOf('K')>=0?m*1000:x.indexOf('M')>=0?m*1000000:m;}
