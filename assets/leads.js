...
function renderLeads() {
  ...
  var folderHome=!folder;renderLeadFolderView(folder);var folderToolbar=document.querySelector('#pgleads>.ptbar'),folderTable=document.querySelector('#pgleads>.twrap'),activeFilters=document.getElementById('activeLeadFilters');if(folderToolbar)folderToolbar.style.display=folderHome?'none':'';if(folderTable)folderTable.style.display=folderHome?'none':'';if(activeFilters)activeFilters.style.display=folderHome?'none':'';
...
document.addEventListener('keydown',function(e){if(e.key!=='Escape')return;var folderEditor=document.getElementById('folderEditorOv');if(folderEditor&&folderEditor.classList.contains('open'))closeLeadFolderEditor();else if(document.getElementById('lbOv').classList.contains('open'))closeLB();else closeLeadFilterMenu();});
