...
function renderLeads() {
  ...
  if(summary){summary.style.display=folderHome?'none':'';var phones=leads.filter(function(l){return l.phone;}).length,emails=leads.filter(function(l){return l.email;}).length,sites=leads.filter(function(l){return l.website;}).length,ready=leads.filter(function(l){return l.phone||l.email;}).length;summary.innerHTML='<div><strong>'+leads.length+'</strong><span>Всички бизнеси</span></div><div><strong>'+ready+'</strong><span>С директен контакт</span></div><div><strong>'+phones+'</strong><span>Телефони</span></div><div><strong>'+emails+'</strong><span>Имейли</span></div><div><strong>'+sites+'</strong><span>Уебсайтове</span></div>';}  if(folderHome){var homeBadge=document.getElementById('pgbdg');homeBadge.style.display='';homeBadge.textContent=getLeadFolderNames().length+' папки';document.getElementById('ltbody').innerHTML='';renderMobileLeadCards([]);renderLeadPager(0,1);return;}
...
function finishOutreachProgress(total,failed){var box=document.getElementById('outreachProgress');if(!box)return;box.innerHTML='<div class="outreachprogresscard finished"><span class="modaleyebrow">КАМПАНИЯТА ПРИКЛЮЧИ</span><h3>'+(failed?'Изпращането завърши с грешки':'Всички писма са изпратени')+'</h3><p><b>'+(total-failed)+'</b> успешни'+(failed?' · <em>'+failed+' неуспешни</em>':'')+'</p><button class="btn btnp" onclick="closeOutreachProgress()">Готово</button></div>';}
function closeOutreachProgress(){var box=document.getElementById('outreachProgress');if(box){box.hidden=true;box.innerHTML='';}renderLeadOutreach();}
...
function outreachRows(segment){var folder=getSelectedLeadFolder();return leads.filter(function(l){if(!folder||l.folder!==folder)return false;var noSite=!String(l.website||'').trim(),hasEmail=!!String(l.email||'').trim(),hasContact=hasEmail||!!String(l.phone||'').trim(),noSocial=!leadHasSocial(l);if(segment==='email_no_site')return noSite&&hasEmail;if(segment==='contact_no_site')return noSite&&hasContact;if(segment==='no_presence')return noSite&&hasContact&&noSocial;if(segment==='ready')return l.outreach&&l.outreach.status==='ready';if(segment==='sent')return l.outreach&&l.outreach.status==='sent';return false;});}
function outreachStatusLabel(l){var s=l.outreach&&l.outreach.status;return s==='sent'?'Изпратен':s==='ready'?'Готов':s==='draft'?'Чернова':'Нов';}
function openLeadOutreach(){var folder=getSelectedLeadFolder();if(!folder){toast('Първо отвори папка с leads','var(--yellow)');return;}var ov=document.getElementById('leadOutreachOv');if(!ov)return;document.getElementById('leadOutreachTitle').textContent='Outreach · '+folder;ov.classList.add('open');document.body.classList.add('outreach-open');outreachSegment='email_no_site';var rows=outreachRows(outreachSegment);outreachLeadId=rows.length?rows[0].id:null;renderLeadOutreach();}
...
function storeOutreach(status){var l=currentOutreachLead();if(!l)return;var d=getOutreachEditor();l.outreach={subject:d.subject.trim(),body:d.body.trim(),status:status,updatedAt:new Date().toISOString(),sentAt:status==='sent'?new Date().toISOString():((l.outreach||{}).sentAt||'')};saveData();renderLeadOutreach();}
