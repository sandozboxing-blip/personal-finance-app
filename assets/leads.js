...
function renderLeadCommunication(l){
...
function openLB(id) {
  ...
  document.getElementById('lbcommunicationPane').innerHTML = renderLeadCommunication(l);
...
function openLatestLeadReply(){
  var folder=getSelectedLeadFolder(),rows=leads.filter(function(l){return l.outreach&&l.outreach.status==='replied'&&(!folder||l.folder===folder);});
  ...
  if(!rows.length)return false;var outreach=document.getElementById('leadOutreachOv');if(outreach&&outreach.classList.contains('open'))closeLeadOutreach();openLB(rows[0].id);return true;
}
function syncOutreachReplies(manual){
  if(outreachReplySyncing||!currentUser)return;outreachReplySyncing=true;var btn=document.getElementById('outreachSyncBtn');if(btn){btn.disabled=true;btn.textContent='Проверяваме…';}
  fetch('api.php?action=syncOutreachReplies',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:'{}'})
    ...
    .then(function(syncResult){return fetch('api.php?action=load',{credentials:'same-origin'}).then(function(r){return r.json().then(function(d){if(!r.ok)throw new Error(d.error||'Данните не се заредиха');return{sync:syncResult,state:d.state||{}};});});})
    .then(function(result){var state=result.state;leads=state.leads||[];leadFolders=state.leadFolders||leadFolders;leadFolderMeta=state.leadFolderMeta||leadFolderMeta;lastServerUpdatedAt=state.updatedAt||lastServerUpdatedAt;normalizeData();saveLocal(false);renderLeads();if(window.renderReports)renderReports();var opened=openLatestLeadReply();if(opened)toast('Показан е последният реален отговор','var(--green)');else if(manual)toast('Няма намерен отговор към lead','var(--blue)');})
