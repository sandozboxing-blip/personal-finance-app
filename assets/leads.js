...
function openLeadOutreach(){var folder=getSelectedLeadFolder();if(!folder){toast('Първо отвори папка с leads','var(--yellow)');return;}var ov=document.getElementById('leadOutreachOv');if(!ov)return;document.getElementById('leadOutreachTitle').textContent='Кампания · '+folder;ov.classList.add('open');document.body.classList.add('outreach-open');outreachSegment='email_no_site';var rows=outreachRows(outreachSegment);outreachLeadId=rows.length?rows[0].id:null;renderLeadOutreach();}
