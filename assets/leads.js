...
function renderLeadCommunication(l){
  var o=l.outreach&&typeof l.outreach==='object'?l.outreach:{},status=o.status||'new',sent=!!o.sentAt,replied=status==='replied',ready=status==='ready'||status==='draft',statusText=replied?'Получен отговор':sent?'Имейлът е изпратен':ready?'Подготвен, но не е изпратен':'Не е изпращан имейл',statusClass=replied?'replied':sent?'sent':ready?'ready':'new',date=function(v){return v?new Date(v).toLocaleString('bg-BG'):'—';};
  return'<section class="lbcommunication '+statusClass+'"><div class="lbcommhead"><div><span>ИМЕЙЛ КОМУНИКАЦИЯ</span><h3>'+statusText+'</h3></div><i>'+outreachStatusLabel(l)+'</i></div><div class="lbcommsteps"><div class="done"><b>1</b><span>Lead добавен</span></div><div class="'+(sent?'done':'')+'"><b>2</b><span>'+(sent?'Изпратен '+date(o.sentAt):'Няма изпращане')+'</span></div><div class="'+(replied?'done reply':'')+'"><b>3</b><span>'+(replied?'Отговор '+date(o.replyAt):'Няма отговор')+'</span></div></div><div class="lbcommdetails"><div><span>Получател</span><strong>'+esc(l.email||'Няма имейл')+'</strong></div>'+(o.subject?'<div><span>Изпратена тема</span><strong>'+esc(o.subject)+'</strong></div>':'')+(o.body?'<details><summary>Виж изпратеното съобщение</summary><p>'+esc(o.body).replace(/\n/g,'<br>')+'</p></details>':'')+(replied?'<div class="lbcommreply"><span>Тема на отговора</span><strong>'+esc(o.replySubject||'Без тема')+'</strong><small>Пълният текст е във входящата поща на '+esc(l.email)+'.</small></div>':'')+(o.sendError?'<div class="lbcommerror"><span>Грешка при изпращане</span><strong>'+esc(o.sendError)+'</strong></div>':'')+(l.followup?'<div><span>Следващ follow-up</span><strong>'+fmtD(l.followup)+'</strong></div>':'')+'</div><div class="lbcommactions">'+(l.email?'<a class="btn btng btnsm" href="mailto:'+esc(l.email)+'">Напиши имейл</a>':'')+'<button class="btn btng btnsm" onclick="syncOutreachReplies(true)">Провери за отговор</button></div></section>';
}
function openLB(id) {
