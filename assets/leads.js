...
function syncOutreachReplies(manual){if(outreachReplySyncing||!currentUser)return;outreachReplySyncing=true;var btn=document.getElementById('outreachSyncBtn');if(btn){btn.disabled=true;btn.textContent='Проверяваме…';}fetch('api.php?action=syncOutreachReplies',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json'},body:'{}'}).then(function(r){return r.json().catch(function(){return{};}).then(function(d){if(!r.ok)throw new Error(d.error||'IMAP грешка');return d;});}).then(function(d){if(d.updated){outreachSegment='replied';outreachScopeIds=null;return fetch('api.php?action=load',{credentials:'same-origin'}).then(function(r){return r.json();}).then(function(x){var state=x.state||{};leads=state.leads||leads;lastServerUpdatedAt=state.updatedAt||lastServerUpdatedAt;normalizeData();saveLocal(false);renderLeads();renderLeadOutreach();if(lbid)openLB(lbid);else{var activeFolder=getSelectedLeadFolder(),latestReply=leads.filter(function(l){return l.folder===activeFolder&&l.outreach&&l.outreach.status==='replied';}).sort(function(a,b){return String((b.outreach||{}).replyAt||'').localeCompare(String((a.outreach||{}).replyAt||''));})[0];if(latestReply){closeLeadOutreach();openLB(latestReply.id);}}if(window.renderReports)renderReports();toast('Отворен е последният получен отговор','var(--green)');});}if(manual){var currentFolder=getSelectedLeadFolder(),existingReply=leads.filter(function(l){return l.folder===currentFolder&&l.outreach&&l.outreach.status==='replied';}).sort(function(a,b){return String((b.outreach||{}).replyAt||'').localeCompare(String((a.outreach||{}).replyAt||''));})[0];if(existingReply&&!lbid){closeLeadOutreach();openLB(existingReply.id);toast('Отворен е последният получен отговор','var(--green)');}else toast('Няма по-нов отговор','var(--blue)');}}).catch(function(e){if(manual)toast(e.message||'Грешка при IMAP проверката','var(--red)');}).finally(function(){outreachReplySyncing=false;if(btn){btn.disabled=false;btn.textContent='Провери отговори';}});}
