...
function toggleTask(id,occurrenceIso){var tasks=getTasks(),day=occurrenceIso||taskSelectedDate||taskIso(new Date());tasks.forEach(function(t){if(t.id!==id)return;if((t.repeat||'none')!=='none'){t.completedDates=Array.isArray(t.completedDates)?t.completedDates:[];var i=t.completedDates.indexOf(day);if(i>=0)t.completedDates.splice(i,1);else t.completedDates.push(day);t.done=false;}else{t.done=!t.done;t.completedAtDate=t.done?day:'';}});saveTasks(tasks);renderTasks();renderTaskManager();}
function deleteTask(id){saveTasks(getTasks().filter(function(t){return t.id!==id;}));renderTasks();renderTaskManager();}
function renderTasks(){var el=document.getElementById('taskList');if(!el)return;var tasks=getTasks(),today=taskIso(new Date());el.innerHTML=tasks.length?tasks.map(function(t){var complete=taskIsCompleteOn(t,today);return '<label class="task '+(complete?'done':'')+'"><input type="checkbox" '+(complete?'checked':'')+' onchange="toggleTask(\''+t.id+'\',\''+today+'\')"><span>'+esc(t.text)+'</span><button type="button" aria-label="Изтрий задача" onclick="event.preventDefault();deleteTask(\''+t.id+'\')">×</button></label>';}).join(''):'<div class="emptymini">Добави до 3 важни задачи за деня.</div>';}
