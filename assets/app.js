...
var leads=[],smm=[],web=[],sharedTasks=[],focusTasks=[],taskCategories=[],userSettings={},currentUser='',syncTimer=null;
...
function saveLocal(){try{var k=currentUser||'guest';localStorage.setItem('d8l',JSON.stringify(leads));localStorage.setItem('d8s2',JSON.stringify(smm));localStorage.setItem('d8w',JSON.stringify(web));localStorage.setItem('d8tasks:'+k,JSON.stringify(sharedTasks));localStorage.setItem('d8focus:'+k,JSON.stringify(focusTasks));localStorage.setItem('d8taskcats:'+k,JSON.stringify(taskCategories));localStorage.setItem('d8settings:'+k,JSON.stringify(userSettings));}catch(e){}}
...
function loadData(){var k=currentUser||'guest';try{leads=JSON.parse(localStorage.getItem('d8l')||'[]');smm=JSON.parse(localStorage.getItem('d8s2')||'[]');web=JSON.parse(localStorage.getItem('d8w')||'[]');sharedTasks=JSON.parse(localStorage.getItem('d8tasks:'+k)||'[]');focusTasks=JSON.parse(localStorage.getItem('d8focus:'+k)||'[]');taskCategories=JSON.parse(localStorage.getItem('d8taskcats:'+k)||'[]');userSettings=JSON.parse(localStorage.getItem('d8settings:'+k)||'{}');}catch(e){leads=[];smm=[];web=[];sharedTasks=[];focusTasks=[];taskCategories=[];userSettings={};}normalizeData();fetch('api.php?action=load',{credentials:'same-origin'}).then(function(r){if(r.status===401){location.reload();throw new Error('auth');}return r.json();}).then(function(d){var state=d.state||{},serverHasData=!!state.updatedAt||(state.leads||[]).length||(state.smm||[]).length||(state.web||[]).length||(state.tasks||[]).length;if(serverHasData){leads=state.leads||[];smm=state.smm||[];web=state.web||[];sharedTasks=state.tasks||[];focusTasks=state.focusTasks||[];taskCategories=state.taskCategories||[];userSettings=state.settings||{};if(!state.focusInitialized){var migratedFocus=sharedTasks.filter(function(t){return !t.due&&!t.category&&(t.repeat||'none')==='none';});if(migratedFocus.length){focusTasks=migratedFocus.map(function(t){return{id:t.id,text:t.text||'',done:!!t.done};});sharedTasks=sharedTasks.filter(function(t){return migratedFocus.indexOf(t)<0;});}saveData();}normalizeData();saveLocal();}else if(leads.length||smm.length||web.length||sharedTasks.length||focusTasks.length){saveData();}populateCats();renderTaskCategories();applyProfileSettings();updateBadges();if(curpg==='dash')renderDash();if(curpg==='smm')renderSmm();if(curpg==='web')renderWeb();if(curpg==='leads')renderLeads();if(curpg==='tasks')renderTaskManager();}).catch(function(e){if(e.message!=='auth')toast('⚠ Работа офлайн — промените се пазят на това устройство','var(--yellow)');});}
// ── NAVIGATION ─────────────────────────────────────────
...
function saveLeadPriorities(){localStorage.setItem('d8LeadPriority:'+(currentUser||'guest'),JSON.stringify(leadPriorityRules));userSettings.leadPriorityRules=leadPriorityRules.slice();saveData();renderLeadPriorityRules();renderLeads();}
...
function exportFullBackup() {
  var state = {version:3, exportedAt:new Date().toISOString(), leads:leads, smm:smm, web:web, tasks:sharedTasks, focusTasks:focusTasks, taskCategories:taskCategories, settings:userSettings};
