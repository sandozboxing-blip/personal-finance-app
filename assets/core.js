...
function paidCycles(c){return c&&c.paidThrough?billingCycles(c.start,c.duration,c.paidThrough):0;}
function smmEarned(c){return paidCycles(c)*((parseFloat(c.monthly)||0)-(parseFloat(c.cost)||0));}
function webEarned(c){var monthly=(c.paymentType||'monthly')==='monthly';if(!monthly)return c.oneTimePaid===true?((parseFloat(c.oneTime)||0)-(parseFloat(c.cost)||0)):0;var initial=c.initialPaid===true?(parseFloat(c.initial)||0):0;return initial+paidCycles(c)*((parseFloat(c.monthly)||0)-(parseFloat(c.cost)||0));}
...
function renderDash(){
 var activeSmm=smm.filter(function(c){return c.status==='active';}),activeWeb=web.filter(function(c){return c.status==='active';}),monthlyWeb=activeWeb.filter(function(c){return(c.paymentType||'monthly')==='monthly';}),oneWeb=web.filter(function(c){return c.status==='completed'&&c.paymentType==='one_time'&&c.oneTimePaid===true;});
 var smmMrr=activeSmm.reduce(function(x,c){return x+(parseFloat(c.monthly)||0);},0),webMrr=monthlyWeb.reduce(function(x,c){return x+(parseFloat(c.monthly)||0);},0),webInitial=web.reduce(function(x,c){return x+(c.initialPaid===true?(parseFloat(c.initial)||0):0);},0),webOne=oneWeb.reduce(function(x,c){return x+(parseFloat(c.oneTime)||0);},0),netToDate=smm.reduce(function(x,c){return x+smmEarned(c);},0)+web.reduce(function(x,c){return x+webEarned(c);},0),ready=leads.filter(function(l){return l.phone||l.email;}).length,today=new Date();today.setHours(0,0,0,0);
